...
def train_dataset(_epoch, dataloader, model, loss_function, optimizer, att_model = False):
    ...
    for index, batch in enumerate(dataloader, 1):
        ...
        ## transform labels to the same shape as logits
        if DATASET == "MR":
            new = torch.FloatTensor([[1,0] if l==0 else [0,1] for l in labels])
            labels = new
        ## calculate error
...
def eval_dataset(dataloader, model, loss_function, att_model = False):
    ...
    with torch.no_grad():
        for index, batch in enumerate(dataloader, 1):
            ...
            ## transform as previsously
            if DATASET == "MR":
                new = torch.FloatTensor([[1,0] if l==0 else [0,1] for l in labels])
            else:
                new = labels
            
            loss = loss_function(logits,new) # EX9

            # Step 4 - make predictions (class = argmax of posteriors)
            pred = torch.argmax(logits,dim=1) # EX9

            # Step 5 - collect the predictions, gold labels and batch loss
            y_pred.append(pred.numpy())  # EX9      ## numpy(): turn torch object to numpy
            y.append(new.numpy())
